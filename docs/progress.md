...
- [x] `src/features/build_tfidf.py` — TF-IDF + cosine similarity (**TỰ CODE**, không sklearn)
- [x] `src/features/build_spmi.py` — co-occurrence → PMI → SPMI → tune k trên train → save
- [x] `src/features/build_knowledge_graph.py` — graph → node2vec (**TỰ CODE**, không node2vec/gensim/sklearn) → tune trên train → save
...
- [x] **Refactor: Loại bỏ thư viện ML có sẵn** — tự code TF-IDF, node2vec, skip-gram, cosine similarity (xem [#refactor-ml-libs] bên dưới)
...
| `src/features/build_tfidf.py` | ✅ **Tự code** | ~330 dòng, TF-IDF + cosine similarity (numpy+scipy) |
| `src/features/build_spmi.py` | ✅ Đã tạo | ~370 dòng, co-occurrence → SPMI → tune k (đã tự code từ đầu) |
| `src/features/build_knowledge_graph.py` | ✅ **Tự code** | ~520 dòng, graph → random walks → skip-gram → tune params |
| `src/features/build_hybrid.py` | ✅ Đã tạo | ~430 dòng, normalize → grid search α/β/threshold (đã tự code từ đầu) |
