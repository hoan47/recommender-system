...
#### Cải tiến: Ensemble Count Vectorizer (Jaccard) + TF-IDF

CB hiện tại sử dụng **ensemble của Count Vectorizer (Jaccard similarity) và TF-IDF (Cosine similarity)**:

```python
final_similarity = alpha × Jaccard(Count) + (1 - alpha) × Cosine(TF-IDF)
```

| Thành phần | Vai trò |
|---|---|
| **Count Vectorizer** (raw count → Jaccard) | Đếm từ đơn tuyệt đối: càng trùng nhiều từ → điểm càng tiến sát 1.0. Bất chấp độ dài câu hay trật tự từ |
| **TF-IDF** (Cosine) | Phân hóa thông minh: từ khóa chính (Cá hồi, Bắp bò) ↑, từ phụ (gói, hộp) ↓ |
| **Jaccard metric** (`CB_METRIC = 'jaccard'`) | `intersection / union` trên nhị phân hóa — tuyến tính theo số từ trùng |
| **Alpha** (`CB_ALPHA = 1.0`) | Hoàn toàn dùng Jaccard Count; có thể giảm để pha TF-IDF |

Cả 2 ma trận đều là sparse CSR, lưu riêng thành 2 file:
- `tfidf_vectors.npz` — ma trận TF-IDF (đã L2-normalized)
- `count_vectors.npz` — ma trận Count raw (chưa normalize, dùng cho Jaccard)
