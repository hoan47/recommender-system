...
# TF-IDF trên product_name + one-hot aisle_id + one-hot department_id
...
**Bước 2 — Random Walk**:
Node2Vec thực hiện random walk có kiểm soát trên đồ thị, tạo ra các "câu" (chuỗi node). Tham số `p` và `q` kiểm soát hành vi walk:
- `p` nhỏ → walk có xu hướng quay lại node cũ (BFS-like, học cấu trúc local)
- `q` nhỏ → walk có xu hướng đi xa (DFS-like, học cấu trúc global)
...
- Cần tuning: edge threshold, tham số p/q, số bước walk
...
| **Node2Vec** | Mối quan hệ bậc cao trong cấu trúc đồ thị sản phẩm |
...
| Tiêu chí | Ochiai+Conf | Item2Vec | Node2Vec | Assoc. Rules | CB Filter | Ensemble (w/o CB) | Ensemble + CB |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Vai trò** | Model gợi ý | Model gợi ý | Model gợi ý | Baseline | Bộ lọc hậu xử lý | Ensemble thuần (O+I+N) | Ensemble + Filter |
...
| **4** | **Node2Vec** | Bổ sung high-order relationships, tốn thời gian nhất |
...
Với mỗi model cần đánh giá (Ochiai, Item2Vec, Node2Vec, Association Rules, Ensemble+CB), mẫu được chọn theo tỷ lệ:
...
| `model_name` | str | Tên model tạo ra mẫu này (`ochiai`, `item2vec`, `node2vec`, `assoc_rules`, `ensemble_cb`) |
