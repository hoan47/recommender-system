...
```
                    ...
                    ▼                    ▼                    ▼
          ┌──────────────────┐  ┌──────────────────┐  ┌──────────────┐
           │ Item-CF          │  │ Item2Vec         │  │ Metapath2Vec │
           │ (Item-Based CF,  │  │ (Word2Vec        │  │ (IKG         │
           │  score có hướng) │  │  trên giỏ hàng)  │  │  embedding)  │
          └────────┬─────────┘  └────────┬─────────┘  └──────┬───────┘
                   │                     │                   │
                   └──────────┬──────────┴──────────┬────────┘
                              │                     │
...
### 4.4 Graph-based: Metapath2Vec (IKG Embedding + Metapath Walk)

**Ý tưởng**: Nâng cấp từ DeepWalk lên **Đồ thị Tri thức đa thể (Heterogeneous Knowledge Graph - IKG)** với 3 loại nút (Product, Aisle, Department) và 3 loại quan hệ (CO_OCCUR, BELONGS_TO, PART_OF). Sử dụng **Metapath Walk** — duyệt đồ thị định hướng ngữ nghĩa — thay vì uniform random walk để giải quyết bài toán siêu nút (supernode) và long-tail.
...
**Bước 1 — Xây IKG**:
- Đồ thị CO_OCCUR: từ co-occurrence count (giống DeepWalk cũ)
...
**Mục tiêu:**
ARM đóng vai trò điểm tham chiếu (baseline). Tất cả các model — ARM, Item-CF, Item2Vec, Ensemble — đều được chấm điểm bằng **cùng một bộ ground truth do LLM sinh ra**, sau đó so sánh kết quả với nhau.
...
Do dữ liệu quá lớn (3.4M orders × 49K products) để chạy FP-Growth/Apriori truyền thống, Association Rules được **tự implement trực tiếp từ co-occurrence matrix** (CSR sparse matrix) đã xây dựng ở ItemCFModel:
...
| **Metapath2Vec** | Mối quan hệ bậc cao trong cấu trúc đồ thị IKG, xử lý long-tail nhờ semantic walk |
...
| Tiêu chí | Item-CF | Item2Vec | Metapath2Vec | Assoc. Rules | CB Filter | Ensemble (w/o CB) | Ensemble + CB |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **Vai trò** | Model gợi ý | Model gợi ý | Model gợi ý | Baseline | Bộ lọc hậu xử lý | Ensemble thuần (ICF+I2V+MW) | Ensemble + Filter |
| **LLM Eval (Precision@10)** | ? | ? | ? | ? | — ⚠️ | ? | ? (kỳ vọng cao nhất) |
| **Loại bỏ substitute** | ❌ | ❌ | ❌ | ❌ | ✅ Tốt | ❌ | ✅ Tốt |
| **Item phổ biến (cơm+gà)** | ✅ Item-CF fix | ✅ | ✅ | ⚠️ | — | ✅ | ✅ |
...
| **4** | **Metapath2Vec** | Xây IKG + Metapath Walk, bổ sung high-order relationships + xử lý long-tail |
...
Với mỗi model cần đánh giá (Item-CF, Item2Vec, Metapath2Vec, Association Rules, Ensemble+CB), mẫu được chọn theo tỷ lệ:
...
| `model_name` | str | Tên model tạo ra mẫu này (`item_cf`, `item2vec`, `metapath2vec`, `assoc_rules`, `ensemble_cb`) |
