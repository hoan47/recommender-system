...
#### Cải tiến: Ensemble Count Vectorizer + TF-IDF

CB hiện tại sử dụng **ensemble của cả Count Vectorizer và TF-IDF** để tính similarity giữa 2 sản phẩm:

```python
final_similarity = alpha × cosine_sim(Count) + (1 - alpha) × cosine_sim(TF-IDF)
```

| Thành phần | Vai trò |
|---|---|
| **Count Vectorizer** (L2-normalized) | Đảm bảo càng trùng nhiều từ → điểm càng cao (tuyến tính theo số lượng) |
| **TF-IDF** | Phân hóa thông minh: từ khóa chính (Cá hồi, Bắp bò) ↑, từ phụ (gói, hộp) ↓ |
| **Alpha** (`CB_ALPHA = 0.5`) | Cân bằng giữa 2 phương pháp |

Cả 2 ma trận đều là sparse CSR, lưu riêng thành 2 file:
- `tfidf_vectors.npz` — ma trận TF-IDF
- `count_vectors.npz` — ma trận Count (L2-normalized)
