...
from config import MIN_CONF, MIN_LIFT, ASYMMETRY_RATIO, PATH_DEPT_DIR

dept_suggest: dict  = None
conf_lookup: dict   = {}
lift_lookup: dict   = {}
...
def build() -> None:
    ...
    print("\n" + "=" * 65)
    print("BUILD — Department association rules")
    print("=" * 65)
    ...
    dept_basket = (
        ...
        .dropna(subset=["department_id"])[["order_id", "department_id"]]
        .drop_duplicates()
    ...
    for did in dl.dept_ids:
        targets = [
            dl.dept_name.get(t, str(t))
            for t in sorted(dept_suggest.get(did, set()))
            if t != did
        ]
        if targets:
            print(f"  {dl.dept_name.get(did, '?'):<20} -> {', '.join(targets)}")

    pd.DataFrame(direction_log).to_csv(PATH_DEPT_DIR, index=False, encoding="utf-8-sig")
    print(f"\n  Saved -> {PATH_DEPT_DIR}")
...
def filter_by_direction(seed_pid: int, candidate_pids: list) -> list:
    ...
    return [p for p in candidate_pids if dl.prod_dept_map.get(p, -1) in allowed_dept]
